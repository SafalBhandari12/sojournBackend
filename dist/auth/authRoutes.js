...
import { otpRateLimit, loginRateLimit, vendorRegistrationRateLimit, generalRateLimit, } from "../middleware/validation.js";
import { authMiddleware, optionalAuth, authorize, authorizeVendor, authorizeAdmin, } from "../middleware/auth.js";
 ...
 */
router.post("/vendor/register", authMiddleware, vendorRegistrationRateLimit, AuthController.registerVendor);
/**
 ...
 */
router.put("/vendor/profile", authMiddleware, authorize("VENDOR"), AuthController.updateVendorProfile);
// ================================
 ...
 */
router.get("/admin/vendors", authMiddleware, authorizeAdmin, AuthController.getVendorsForAdmin);
/**
 * @route PUT /api/auth/admin/vendor/:vendorId/approve
 * @desc Approve vendor application
 * @access Private (Admin only)
 */
router.put("/admin/vendor/:vendorId/approve", authMiddleware, authorizeAdmin, AuthController.approveVendor);
/**
 * @route PUT /api/auth/admin/vendor/:vendorId/reject
 * @desc Reject vendor application
 * @access Private (Admin only)
 */
router.put("/admin/vendor/:vendorId/reject", authMiddleware, authorizeAdmin, AuthController.rejectVendor);
/**
 * @route PUT /api/auth/admin/vendor/:vendorId/suspend
 * @desc Suspend vendor
 * @access Private (Admin only)
 */
router.put("/admin/vendor/:vendorId/suspend", authMiddleware, authorizeAdmin, AuthController.suspendVendor);
/**
 * @route PUT /api/auth/admin/user/:userId/assign-admin
 * @desc Assign admin role to a user
 * @access Private (Admin only)
 */
router.put("/admin/user/:userId/assign-admin", authMiddleware, authorizeAdmin, AuthController.assignAdminRole);
/**
 * @route PUT /api/auth/admin/user/:userId/revoke-admin
 * @desc Revoke admin role from a user
 * @access Private (Admin only)
 */
router.put("/admin/user/:userId/revoke-admin", authMiddleware, authorizeAdmin, AuthController.revokeAdminRole);
/**
 * @route GET /api/auth/admin/users
 * @desc Get all users for admin review
 * @access Private (Admin only)
 */
router.get("/admin/users", authMiddleware, authorizeAdmin, AuthController.getAllUsers);
/**
 * @route PUT /api/auth/admin/user/:userId/toggle-status
 * @desc Activate or deactivate a user account
 * @access Private (Admin only)
 */
router.put("/admin/user/:userId/toggle-status", authMiddleware, authorizeAdmin, AuthController.toggleUserStatus);
/**
 * @route PUT /api/auth/admin/profile
 * @desc Update admin profile information
 * @access Private (Admin only)
 */
router.put("/admin/profile", authMiddleware, authorizeAdmin, AuthController.updateAdminProfile);
