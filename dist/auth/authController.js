...
import { sendOTPSchema, verifyOTPSchema, resendOTPSchema, vendorRegistrationSchema, adminAssignmentSchema, toggleStatusSchema, adminProfileUpdateSchema, validateRequest, } from "./validator.js";
import { getOTPConfig, sendOTPToMessageCentral, validateOTPWithMessageCentral, createOTPRecord, findOrCreateUser, hasActiveOTP, markPreviousOTPsAsUsed, findOTPRecord, markOTPAsVerified, incrementOTPAttempts, } from "./otpUtils.js";
...
class AuthController {
    ...
    static async sendOTP(req, res) {
        try {
            ...
            if (otpResult.success && otpResult.data) {
                ...
                // Store OTP record
                await createOTPRecord(user.id, otpResult.data.mobileNumber, otpResult.data.verificationId, otpResult.data.timeout);
    ...
    static async verifyOTP(req, res) {
        try {
            ...
            if (dummyPhoneNumbers.includes(phoneNumber)) {
                ...
                }, process.env.JWT_SECRET || "your-secret-key", { expiresIn: "15m" } // Short-lived access token (15 minutes)
                );
                ...
                }, process.env.JWT_SECRET || "your-secret-key", { expiresIn: "30d" } // Maximum lifetime (30 days)
                );
                return res.status(200).json({
                    success: true,
            ...
            const validateResult = await validateOTPWithMessageCentral(phoneNumber, verificationId, code, config);
    ...
    static async resendOTP(req, res) {
        try {
            ...
            if (otpResult.success && otpResult.data) {
                ...
                await createOTPRecord(existingUser.id, otpResult.data.mobileNumber, otpResult.data.verificationId, otpResult.data.timeout);
                return res.status(200).json({
                    success: true,
    ...
    static async registerVendor(req, res) {
        try {
            ...
            const { businessName, ownerName, contactNumbers, email, businessAddress, googleMapsLink, gstNumber, panNumber, aadhaarNumber, vendorType, bankDetails, } = validation.data;
     ...
     * Get vendor status
     */
    static async getVendorStatus(req, res) {
        try {
            if (!req.user) {
                return res.status(401).json({
                    success: false,
            ...
            if (!vendor) {
                return res.status(200).json({
                    ...
                    data: {
                        status: "NOT_APPLIED",
                        message: "No vendor application found. You can apply to become a vendor.",
                    },
                });
            }
            return res.status(200).json({
    ...
    static async assignAdminRole(req, res) {
        try {
            ...
            const adminProfile = await AuthController.createAdminProfile(userId, fullName || "Admin User", email || undefined, permissions || ["MANAGE_VENDORS", "MANAGE_USERS"]);
            return res.status(200).json({
                success: true,
     ...
     */
    static async revokeAdminRole(req, res) {
        try {
            if (!req.user || req.user.role !== "ADMIN") {
                return res.status(403).json({
                    success: false,
     ...
     */
    static async createAdminProfile(userId, fullName, email, permissions = ["MANAGE_VENDORS", "MANAGE_USERS"]) {
        ...
        try {
            ...
            try {
                ...
                if (refreshCount >= maxRefreshes) {
                    return res.status(401).json({
                        success: false,
                        message: "Refresh token has reached maximum usage limit. Please login again.",
                    });
                }
                ...
                const newAccessToken = jwt.sign({
                    ...
                    role: user.role,
                    type: "access", // Mark as access token
                }, process.env.JWT_SECRET || "your-secret-key", { expiresIn: "15m" } // Very short-lived access token (15 minutes)
                );
                ...
                const newRefreshToken = jwt.sign({
                    ...
                    expiresIn: Math.floor(maxTokenAge - (currentTime - originalIssuedAt)), // Remaining time until absolute expiration
                });
                return res.status(200).json({
                    success: true,
